...
class Preprocessor:
    ...
    def noun_extractor(self,news,stopWord):
        nouns = []
    ...
    def compute_NUM_TOPICS(self,corpus,id2word,bigram_document,t_min=5,t_max=10):

        coherence_score = []
        for i in range(t_min, t_max):
            # model = gensim.models.ldamodel.LdaModel(corpus = corpus, id2word=id2word, num_topics=i,iterations=50, passes= 30)
            model = gensim.models.ldamodel.LdaModel(corpus = corpus, id2word=id2word, num_topics=i,iterations=20, passes= 30)
    ...
    def cluster_extract_sentences(self,ldamodel,idx_topic,corpus,news,id_news,NUM_TOPICS,id2word,bigram_model, stopWord):
